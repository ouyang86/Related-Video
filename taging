                ...
                for item in self.tag['movie']:                                        #detecting the longest/maximal matching film name from text content
                    if simName(item) in name:
                        ...
                        if refine in item:
                            include = True
                for rem in clean:
                    self.tag['movie'].remove(rem)                    
                if include == False: self.tag['movie'].add(name)
                ...
                for item in self.tag['tv']:                                        #detecting the longest/maximal matching tv show name from text content
                    if simName(item) in name:
                        ...
                        if refine in item:
                            include = True
                for rem in clean:
                    self.tag['tv'].remove(rem)                    
                if include == False: self.tag['tv'].add(name)       

    def similarVideo(self, n):
        self.close = []
        for compare in videoset:
            ...
            sim = sim1 + sim2 + sim3 + sim4                                                             #general estimate of relation strength of each pair of video
            if len(self.close) < n:
                self.close.append((compare, sim))
                self.close.sort(key = lambda x : x[1], reverse = True)
            else:
                if sim > self.close[-1][1]:                                                             #record the top 3 related videos
                    self.close.pop()
                    self.close.append((compare,sim))                                                
                    self.close.sort(key = lambda x : x[1], reverse = True)
...
def addTag(jsondata, start = 0, end = len(jsondata)):
    videoset = []
    tagvideo = open('tag.csv', 'a')
    tagvideo.write('Actor,Movie,TV Show\n')
    for item in jsondata[start:end]:
...
def relate(videoset, start = 0, end = len(videoset)):
    out = open('output.txt', 'a')
    i = 1
    for item in videoset[start: end]:                    #write a file to summerize the results
        item.similarVideo(3)
